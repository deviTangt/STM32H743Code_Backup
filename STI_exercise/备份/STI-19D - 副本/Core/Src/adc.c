...
double ad_val[AD_CHANNEL_NUM];
uint32_t ad_temp[AD_CHANNEL_USE];
/* USER CODE END 0 */
...
float get_GPU_Temperature(uint32_t Timeout)
{
	...
	{
	   Serial_TJC_SendTxt(10, "ADC Conversion failed! \n");
	}
...
void show_GPU_temprature(){
	...
	if (10.0 < Temp_GPU && Temp_GPU < 100.0)
		Serial_TJC_SendTxt(11, "GPU Temp:%f", Temp_GPU);
}
// 显示ADC1四通道数据
void ShowADCResult(void)
{
	...
	for(i = 0; i < AD_CHANNEL_USE;i ++)
		ad_temp[i]= 0;
	
	for(i = 0; i < ADC1_CONVERTED_DATA_BUFFER_SIZE;i ++)
		ad_temp[i % AD_CHANNEL_USE] += ADC1_DATA[i];
	


	for(i = 0; i < AD_CHANNEL_USE;i ++)
		ad_val[i] = ad_temp[i] * 1000 / AD_SAMPLE_NUM * 3.3 / 65536;

	Serial_TJC_SendTxt(5,  "CH3 PA6 val:%4.6fmV \r\n", ad_val[0]);
	Serial_TJC_SendTxt(6,  "CH4 PC4 val:%4.6fmV \r\n", ad_val[1]);
	Serial_TJC_SendTxt(7,  "CH7 PA7 val:%4.6fmV \r\n", ad_val[2]);
	Serial_TJC_SendTxt(8,  "CH8 PC5 val:%4.6fmV \r\n", ad_val[3]);
		
	//Serial_TJC_SendTxt(9,  "CH10 PC0 val: %1.8fV \r\n", ad5*3.3f/65536);
	//Serial_TJC_SendTxt(10, "CH11 PC1 val: %1.8fV \r\n", ad6*3.3f/65536);
}
/* USER CODE END 1 */
